...
nMapRouter.get( "/", async (req: Request, res: Response) => {   //here we create a router for all nmap requests in case we want 
    try {                                                       // to create subroutes for different scan options
        ...
        if (now - lastScan < CACHE_TIMEOUT){                //return cached scan data if the cache hasn't timed out
            console.log("Returning last cached scan");
            res.send(cachedData);
            return;
        }

        if (ongoingScanPromise){                            //if currently scanning then await the ongoing scanPromise
